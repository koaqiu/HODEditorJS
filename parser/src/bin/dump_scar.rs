...
fn extract_pool_from_chunk(chunk: &iff::IffChunk) -> io::Result<()> {
    if chunk.id == "POOL" {
        ...
        let decomp_mesh =
            xpress::decompress(&comp_mesh, decomp_mesh_len as usize).unwrap_or_default();
        ...
        let decomp_face =
            xpress::decompress(&comp_face, decomp_face_len as usize).unwrap_or_default();

        std::fs::write("decompressed_mesh.bin", &decomp_mesh)?;
        std::fs::write("decompressed_face.bin", &decomp_face)?;
        println!(
            "Saved decompressed_mesh.bin ({} bytes) and decompressed_face.bin ({} bytes).",
            decomp_mesh.len(),
            decomp_face.len()
        );
    }
