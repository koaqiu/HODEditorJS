...
| `collision` | At most one `model.collision_meshes[]` entry / COLD+KDOP, normalized as `Root` | Yes | `collision.center`, extents, radius; `collision.mesh.parent_name` is normalized to `Root` for editor hierarchy placement | No | None | Source mesh for auto-bounds calculation | Forbidden. COL nodes are engine-defined and must remain named `Root`; context menu must not expose Rename. | Allowed; removes the single collision hull only, and save/import must not recreate it unless the user adds one. | `HierarchyTree.tsx`, `Inspector.tsx`, `Viewport.tsx`, `parser/src/hod.rs`, `parser/src/dae.rs`, `src-tauri/src/lib.rs` |
...
- COL/collision is a singleton. Loading HOD 1.0, HOD 2.0, or DAE may preserve an existing collision, but parser/import/save/UI normalization must keep only one collision named `Root`; users may delete it but may not rename it.
