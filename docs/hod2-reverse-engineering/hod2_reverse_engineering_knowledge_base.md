...
* **HOD 1.0 Retrofit Caveats**:
  ...
  * HOD 1.0 inline `LMIP` texture payloads need save-time Y-axis compensation when converting to generated HOD 2.0 texture pools. The editor preview can render correctly while the regenerated HOD 2.0 game texture is Y-flipped unless legacy-origin textures are marked and flipped during pool compression only.
