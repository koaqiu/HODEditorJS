...
- ter_pharos test_hodor_replication panics on LMIP integer overflow (`hod.rs:2647`) when parsing HODOR textures with large dimensions.
- Collision mesh vertices generated by `generate_collision_mesh` (box from extents) are not appended to the mesh pool during `generate_v2_from_model`. Mesh pool is 34968 bytes smaller than HODOR, causing vertex explosion in-game.
- Texture compression settings still differ from HODOR (expected — different DXT encoder).
- In-game validation is still required after fixing the collision mesh pool gap.
...
1. Fix LMIP integer overflow in `parse_texture` (`hod.rs:2647`) — cast `width * height` to `usize` before multiply.
2. Fix collision mesh vertex pool appending in `generate_v2_from_model` so mesh pool byte size matches HODOR.
3. Re-run in-game validation after collision mesh pool fix.
4. Expand HODOR source-asset fixtures beyond `ter_pharos` and `ter_centaur`.
