...
## Current Status
- ✅ Verified DAE Parser & Added Triangle Limits Warning: Confirmed the DAE parser losslessly parses original HOD 2.0 models by verifying exact mesh structure, vertex count, face count, and LOD assignment matches using `ter_zephyrus.DAE` vs `ter_zephyrus_2.0_original.hod`. Also added a mesh limits check in `HierarchyTree.tsx`'s `getWarnings` function. The UI now displays an interactive warning if any mesh exceeds the engine limit of 21845 triangles (65535 indices), explicitly advising the user to split the mesh to prevent in-game rendering glitches or crashes.
- ✅ Fixed Companion MAD File Loading on Linux (`parser/src/hod.rs`): Discovered that `companion_mad_candidates` only checked for lowercase `.mad` extensions. On case-sensitive file systems like Linux ext4, this caused the parser to fail to load companion animation files that were saved with an uppercase `.MAD` extension (e.g., `shi_commnode.MAD`). Updated `companion_mad_candidates` to explicitly search for both `.mad` and `.MAD` variants, allowing the backend to correctly parse and deserialize animations to the frontend Hierarchy Tree.
- ✅ Added Node Dragging and Rotating in Viewport (`Viewport.tsx`, `App.tsx`, `Toolbar.tsx`): Enabled direct viewport transformation of nodes (like in Blender) using Three.js `TransformControls`. Added a Move and Rotate mode toggle in the Top Bar. The left click camera rotation is disabled to allow interacting with the gizmos using the left mouse button, while right-click continues to handle camera orbit rotation. Integrated rotation matrix extraction into the `objectChange` event so that dragging the rotation gizmos persists the new orientation back into the HODModel joint representation and React UI state accurately. Also added support for transforming Animation Keyframes, allowing users to select a keyframe and edit its translation and rotation visually within the viewport. Finally, implemented a global `ESC` keydown listener to quickly unselect nodes and detach the gizmos.
...
* **Follow-up Fix**: Adjusted `HierarchyTree.tsx`'s model diagnosis warnings for `weapon_group` so that only `Position` is strictly required to avoid false positive error indicators for minimal Gimbal/Fixed weapons.
* **Follow-up Fix 2**: Added an additional dynamic model diagnosis warning for `weapon_group`. If the weapon assembly has a `Latitude` node (which handles pitch), it must also have a `Muzzle` node to properly fire projectiles. This ensures that users don't create invalid animated turret hierarchies.
* **Follow-up Fix 3**: Fixed a bug in the Texture Group Inspector where changing the sub-texture type (e.g. from `_DIFF` to `_GLOW`) or format would incorrectly rename *all* textures sharing the same name in the group. This happened because the UI matched textures by their `name` string instead of their exact object reference. Updating the match to use strict object reference (`t === item.texture`) resolves this name collision issue and isolates the changes cleanly.

## 2026-06-03: Viewport Gizmo Dragging and TransformControls Fix
* **What failed**: Dragging joints using `TransformControls` caused the joint to snap to `0,0,0` and threw a `TransformControls: The attached 3D object must be a part of the scene graph.` error, crashing the drag.
* **Root Cause 1 (Snap to Origin)**: The Three.js column-major `Matrix4.elements` array was being mapped into the HOD row-major `Matrix4D` array incorrectly. The logical mapping `m[0][0] = elements[0], m[0][1] = elements[4]` was effectively transposing the matrix, placing the translation components into the 4th column instead of the 4th row, causing them to be ignored as 0 when parsed back into translation vectors by `App.tsx`.
* **Root Cause 2 (Scene Graph Crash)**: Dragging triggered continuous `objectChange` events that immediately updated the global React `model` state. The `Viewport.tsx` reacted by tearing down and rebuilding the entire 3D scene mesh tree on every frame. When the drag ended and the scene rebuilt, `scene.updateMatrixWorld(true)` was called *before* `transformControls.detach()` was run, causing Three.js to attempt a matrix calculation on a `targetObj` that had already been removed from the scene.
* **Fix**:
  1. Updated `commitTransform()` in `Viewport.tsx` to read the contiguous 4-element chunks of `elements` to correctly match the HOD row-major `Matrix4D` format.
...
## 2026-06-03: Keyframe Dragging Jitter Fix
* **What failed**: Attempting to drag a joint while a keyframe was selected caused the joint to resist movement, snap back, or move invisibly without dragging its children.
* **Root Cause**: The `Viewport.tsx` animation evaluation loop runs every frame. When a keyframe is active, `evaluateAnimation` forces all joint meshes to match the interpolated transform derived from the raw binary model. This meant that immediately after `TransformControls` moved a mesh, `evaluateAnimation` would instantly teleport it back to the keyframe's original position, fighting the gizmo. Furthermore, `getAnimatedJointGlobalMatrix` returned the global matrix calculated from the outdated model state rather than the live drag state, so child joints could not follow the cursor.
* **Fix**: Added `isBeingDragged` checks inside `evaluateAnimation` and `updateGroupChildren` to entirely skip overwriting the transforms of any `joint`, `marker`, or `navlight` that is currently actively targeted by `TransformControls`. Additionally, updated `getAnimatedJointGlobalMatrix` to intercept and return the live `matrix.clone()` of the `TransformControls.object` when dragging, completely bypassing keyframe evaluation for the dragged node. This allows the dragged node's children to seamlessly inherit the live drag coordinates.
* **Verification**: Gizmos can now freely drag keyframed joints; they smoothly follow the mouse without jitter, and all children follow perfectly during the drag.
