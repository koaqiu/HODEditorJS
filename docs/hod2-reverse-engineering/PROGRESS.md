...
## Current Issues
- **AppImage bundling fails in distrobox containers** (`tauri.conf.json`): Tauri's linuxdeploy AppImage requires FUSE which is unavailable in containers. Workaround: extracted AppImages and created wrapper scripts, but Tauri overwrites wrappers with corrupted data (null bytes in shebang). **Workaround applied**: Changed `"targets": "all"` to `"targets": ["deb", "rpm"]` in `tauri.conf.json`. AppImage bundling disabled until a proper solution is found (e.g., patching Tauri bundler source or using `LINUXDEPLOY` env var if supported).
