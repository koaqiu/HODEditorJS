# Generated HOD Issues Fix Plan (Corrected)
...
## Tool Workflow Clarification

**Correct Workflow:**
```
HOD 1.0 → RODOH → DAE + TGA → HODOR → HOD 2.0
```

- **RODOH.exe** converts HOD 1.0 (Classic) to DAE + TGA (extraction)
- **HODOR.exe** converts DAE + TGA to HOD 2.0 (Remastered) (creation)

**HODOR is the tool that creates HOD 2.0 files**, not RODOH.
...
**Root Cause:** Binormal values are different from HODOR's output  
**Evidence:**
...
**Root Cause:** Texture pool is 33% smaller than HODOR's output  
**Evidence:**
...
**Problem:** The cross product order or handedness calculation is different from HODOR.
...
**Problem:** HODOR applies DXT compression to textures, which is required for correct rendering in game.
...
**Step 1: Analyze HODOR's binormal calculation**
...
**Step 1: Analyze HODOR's texture compression**
...
- Vanilla (HODOR): (-0.038397547, 0.65264744, 0.7566881)

**Texture Pool:**
- Generated: 1,124,443 bytes compressed
- Vanilla (HODOR): 1,673,248 bytes compressed
...
**Probability:** Medium  
**Mitigation:** Match HODOR's compression settings
...
- [ ] Binormal values match vanilla HOD (HODOR output)
- [ ] Texture pool sizes match vanilla HOD (HODOR output)
...
1. **Analyze HODOR's binormal calculation** (1 hour)
2. **Implement binormal fix** (1 hour)
3. **Analyze HODOR's texture compression** (1 hour)
